...
for i = 1:n_poses
    ...
    if i ~= 1
        b(2*i-1:2*i) = sigma_o_inv*[odom(i-1,1);odom(i-1,2)];
    else
        b(i:i+1) = [0;0];
    end
end
...
for i = 1:n_obs
    p_idx = obs(i,1);
    l_idx = obs(i,2);
    A(l_start_idx+2*i-1:l_start_idx+2*i, 2*p_idx-1:2*p_idx) = sigma_l_inv*H_m;
    A(l_start_idx+2*i-1:l_start_idx+2*i, l_start_idx+2*l_idx-1:l_start_idx+2*l_idx) = -sigma_l_inv*H_m;
    b(l_start_idx+2*i-1:l_start_idx+2*i) = sigma_l_inv*[obs(i,3); obs(i,4)];
end
